...
export function KdsView({ branchId, branchName, initialOrders, stations, activeStation }: Props) {
  ...
  return (
    <div className="flex min-h-dynamic-screen flex-col">
      ...
      <main className="flex-1 space-y-6 overflow-y-auto p-6">
        <Section title="New" tone="primary" orders={sections.new} now={now} onUpdate={updateStatus(branchId, setOrders)} branchId={branchId} />
...
function Section({
  ...
  const toneCls = tone === 'primary' ? 'bg-primary/15 text-primary' : tone === 'accent' ? 'bg-accent/20 text-accent-foreground' : 'bg-success/15 text-success';
  return (
    <section>
